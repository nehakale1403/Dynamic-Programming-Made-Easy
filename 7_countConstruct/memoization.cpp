...
int countConstruct(int n, string wordBank[], string target, map<string, int> &memo){
    ...
    for(int i=0; i<n; i++){
        ...
        if(pos == 0){
            ...
            count += countConstruct(n, wordBank, new_target, memo);
        }
    }
    memo[target] = count;
    return count;
...
int main(){
    ...
    cout<<countConstruct(n, wordBank, target, memo);
}